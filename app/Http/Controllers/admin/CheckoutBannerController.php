...
class CheckoutBannerController extends Controller
{
    private $page = "Checkout Banner";

    public function index(){
        $action = "list";
        $banners = BlogBanner::where('estatus',1)->where('page',2)->get();
        ...
        return view('admin.checkoutbanners.list',compact('action','banners','products'))->with('page',$this->page);
    }

    public function create(){
        $action = "create";
        $banners = BlogBanner::where('page',2)->where('estatus',1)->get()->toArray();
        $categories = Category::where('estatus',1)->where('is_custom',0)->get()->toArray();
        $products = Product::where('estatus',1)->where('is_custom',0)->get()->toArray();
        return view('admin.checkoutbanners.list',compact('action','banners','categories','products'))->with('page',$this->page);
    }
    ...
    public function allhomebannerlist(Request $request){
        if ($request->ajax()) {
            ...
            $totalData = BlogBanner::where('page',2)->count();
            ...
            {
                $banners = BlogBanner::where('page',2)->offset($start)
                    ->limit($limit)
                    ->orderBy($order,$dir)
                    ->get();
                ...
                $banners =  BlogBanner::where('page',2)->Where('title', 'LIKE',"%{$search}%")
                    ->offset($start)
                    ->limit($limit)
                    ->orderBy($order,$dir)
                    ->get();
                $totalFiltered = BlogBanner::where('page',2)->Where('title', 'LIKE',"%{$search}%")
                    ->count();
            }
            ...
            if(!empty($banners))
            {
                ...
                {
                    $page_id = ProjectPage::where('route_url','admin.checkoutbanners.list')->pluck('id')->first();
    ...
    public function changecheckoutbannerstatus($id){
    ...
    public function editbanner($id){
        ...
        return view('admin.checkoutbanners.list',compact('action','banner','categories','products'))->with('page',$this->page);
    }
