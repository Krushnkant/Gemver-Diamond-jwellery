        ...
        if($request->ajax()) {
            ...
            if(!empty($products)) {
                foreach($products as $product) {
                    ...
                    $categoriesSlugarray = array();
                    $getCatData = Category::select('category_name', 'slug')->whereIn('id', $primary_category_id)->get()->toArray();
                    // dd($getCatData);
                    foreach($getCatData as $getCatRecord) {
                        ;
                        $categoriesarray[] = $getCatRecord['category_name'];
                        $categoriesSlugarray[] = $getCatRecord['slug'];
                    }
                    // $categoriesarray = DB::table('categories')->select('category_name')->whereIn('id', $primary_category_id)->toArray();
                    // dd($categoriesarray);
                    $categories = implode(', ', $categoriesarray);
                    $categoriesSlug = implode(', ', $categoriesSlugarray);
                    //dd($categories);
                    ...
                    $action = '';
                    $action .= '<a href="'.url('custom-product-details/'.$categoriesSlug.'/'.$product->slug).'" class="btn btn-gray text-orange btn-sm" target="_blank"><i class="fa fa-eye" aria-hidden="true"></i></a>';
                    if(getUSerRole() == 1 || (getUSerRole() != 1 && is_write($page_id))) {
                        $action .= '<button id="editProductBtn" class="btn btn-gray text-blue btn-sm" data-id="'.$product->product_id.'"><i class="fa fa-pencil" aria-hidden="true"></i></button>';
                    }
                    if(getUSerRole() == 1 || (getUSerRole() != 1 && is_delete($page_id))) {
