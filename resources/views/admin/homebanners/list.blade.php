                ...
                <div class="card">
                    <div class="card-body">
                        ...
                        <div class="action-section">
                            <div class="d-flex">
                            <?php $page_id = \App\Models\ProjectPage::where('route_url','admin.homebanners.list')->pluck('id')->first(); ?>
                        ...
                        @if(isset($action) && $action=='create')
                            @include('admin.homebanners.create')
                        @endif

                        @if(isset($action) && $action=='edit')
                            @include('admin.homebanners.edit')
                        @endif
...
$('#BannerInfo').change(function() {
    ...
    if(bannerInfo == 1 || bannerInfo == 2){
        ...
        $.ajax ({
            type:"POST",
            url: "{{ route('admin.homebanners.getBannerInfoVal') }}",
...
$('body').on('click', '#AddBannerBtn', function () {
    location.href = "{{ route('admin.homebanners.add') }}";
});
...
function save_banner(btn,btn_type){
    ...
    $.ajax({
        type: 'POST',
        url: "{{ route('admin.homebanners.save') }}",
        data: formData,
        processData: false,
        contentType: false,
        success: function (res) {
            ...
            if(res.status == 200){
                if(btn_type == 'save_close'){
                    $(btn).prop('disabled',false);
                    $(btn).find('.loadericonfa').hide();
                    location.href="{{ route('admin.homebanners.list')}}";
                    if(res.action == 'add'){
...
$('body').on('click', '#Removebannersubmit', function (e) {
    ...
    $.ajax({
        type: 'GET',
        url: "{{ url('admin/homebanners') }}" +'/' + banner_id +'/delete',
        success: function (res) {
            if(res.status == 200){
...
$('body').on('click', '#editBannerBtn', function () {
    ...
    var url = "{{ url('admin/homebanners') }}" + "/" + blog_id + "/edit";
