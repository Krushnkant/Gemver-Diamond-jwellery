                        ...
                        <form method="post" id="permissionForm" action="">
                            ...
                            <table id="" class="table table-striped customNewtable userPermissionFormTable" style="">
                                ...
                                <tbody>
                                    ...
                                    <input type="hidden" value="{{ $user_permission->user_id }}"
                                        name="permission_user_id">
                                    <input type="hidden" class="project_page_ids"
                                        value="{{ $user_permission->project_page_id }}">
                                    <tr>
                                        <th class="text-center">{{ $i }}</th>
                                        <td>{{ $user_permission->project_page->label }}</td>
                                        <td class="text-center"><input type="checkbox"
                                                class="form-check-input permissionCheckBox"
                                                value="{{ $user_permission->can_read }}" name="canReadArr[]" {{
                                                $user_permission->can_read==1?"checked":'' }} ></td>
                                        <td class="text-center"><input type="checkbox"
                                                class="form-check-input permissionCheckBox"
                                                value="{{ $user_permission->can_write }}" name="canWriteArr[]" {{
                                                $user_permission->can_write==1?"checked":'' }} ></td>
                                        <td class="text-center"><input type="checkbox"
                                                class="form-check-input permissionCheckBox"
                                                value="{{ $user_permission->can_delete }}" name="canDeleteArr[]" {{
                                                $user_permission->can_delete==1?"checked":'' }} ></td>
                                    </tr>
                                    <?php $i++; ?>
                                    @endforeach
                                    <tr>
                                        <td colspan="5" class="text-center"><button type="submit" id="savePermissionBtn"
                                                class="btn btn-primary">Save <i
